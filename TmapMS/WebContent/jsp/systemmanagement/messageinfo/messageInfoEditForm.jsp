...
<script>
	...
	function save() {
		...
		if (confirm("수정하시겠습니까?")) {
			ContentsForm.action = "messageInfoUpdate.do";
			ContentsForm.submit();
		}
	}
	...
	if(state == 'N'){
		if (confirm("삭제하시겠습니까?")) {
			ContentsForm.action = "messageInfoDelete.do";
			ContentsForm.submit();
		}
	}else{
		alert('사용중인 메세지는 삭제할 수없습니다.');
	}
	}
</script>
</head>
<body>
	<form name="ContentsForm" method="post" id="ContentsForm">
		<input type="hidden" name="messageSeq"
			value="${modifyInfo.messageSeq}" />
		...
		<div id="bodyDiv">
			<div class="contentsDiv">
				<div class="contentDiv">
					<table>
						<tr>
							<td width="560"><img src="images/h3_ico.gif"><span>공통코드메세지
									수정</span></td>
							<td class="btn"><a href="javascript:save();"
								onFocus="blur()"><img src="images/btn/modify7.gif"
									border="0"></a> <a href="javascript:del('${modifyInfo.useState}');" onFocus="blur()"><img
									src="images/btn/delete7.gif" border="0"></a> <a
								href="messageInfoList.do" onFocus="blur()"><img
					...
					<table class="srvRegi">
						...
						<tr>
							...
							<td class="first">${modifyInfo.messageType}</td>
						</tr>
						<tr>
							<th>메세지 코드</th>
							<td>${modifyInfo.messageCode}</td>
						</tr>
						<tr>
							<th>메세지 코드값</th>
							<td>${modifyInfo.messageCodeValue}</td>
						</tr>
						<tr>
							<th>메세지 내용</th>
							<td><input type="text" name="messageContent"
								value="${modifyInfo.messageContent}" class="textbox"
						...
						<tr>
							...
							<input type="radio" name="useState" value="Y"
											<c:if test="${modifyInfo.useState == 'Y'}">checked="checked"</c:if> />
											사용 &nbsp;&nbsp; <input type="radio" name="useState" value="N"
											<c:if test="${modifyInfo.useState == 'N'}">checked="checked"</c:if> />
											미사용</td>
						</tr>
					</table>
							...
							<td align="center" background="images/box2.gif"><table
								...
								<table width="748" border="0" cellspacing="0" cellpadding="0">
									<tr>
										...
										<td><input type="text" name="messageCodeValue"
											value="${modifyInfo.messageCodeValue}"
								...
								<table width="748" border="0" cellspacing="0" cellpadding="0">
									<tr>
										...
										<td><input type="text" name="messageContent"
											value="${modifyInfo.messageContent}" class="textbox"
											style="width: 600px" title="메세지내용" maxbyte="200" /></td>
									</tr>
									<tr>
										<td height="1" colspan="4" bgcolor="#f3961d"></td>
									</tr>
								...
								<table width="748" border="0" cellspacing="0" cellpadding="0">
									<tr>
										...
										<td><select name="useState" class="textbox"
											style="width: 100px;">
												<option value="Y"
													<c:if test="${modifyInfo.useState == 'Y'}">selected="selected"</c:if>>사용</option>
												<option value="N"
													<c:if test="${modifyInfo.useState == 'N'}">selected="selected"</c:if>>미사용</option>
										</select></td>
									</tr>
								</table></td>
						</tr>
