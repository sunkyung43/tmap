...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		//조회 버튼 엔터 이벤트 처리
		$("#sComCode").bind("keydown", function(event) {
			if (event.keyCode == 13) {
				searchPage(0);
			}
		});

		$("#codeName").bind("keydown", function(event) {
			if (event.keyCode == 13) {
				searchPage(0);
			}
		});
	...
	function executeNew() {
		$("#ContentsForm").attr('action', 'comCodeInfoNewForm.do');
		$("#ContentsForm").submit();
	}

	function executeDetail(comCode) {
		$("#comCode").val(comCode);
		$("#ContentsForm").attr('action', 'comCodeInfoDetail.do');
		$("#ContentsForm").submit();
	}
	...
	function searchPage(page) {
		...
		$("#ContentsForm").attr('action', 'comCodeInfoList.do');
		$("#ContentsForm").submit();
	};
</script>
</head>
<body>
	<form:form commandName="ContentsForm">
		...
		<form:hidden path="comCode" />

		<jsp:include page="../../common/menu.jsp" />
		<div id="bodyDiv">
			<div class="contentsDiv">
				<div class="contentDiv">
				<img src="images/h3_ico.gif"><span>공통코드 관리</span><br></br>
								...
								<c:otherwise>
									<c:forEach var="list" items="${comCodeList}">
										<tr
											...
											onclick="javascript:executeDetail('${list.comCode}');">
											<td class="td03">${list.rownum}</td>
											<td class="td03">${list.comCode}</td>
											<td class="td03">${list.codeName}</td>
											<td class="td03">${list.codeRemark}</td>
											<td class="td03"><c:if test="${list.useState == 'Y'}">사용</c:if>
												<c:if test="${list.useState == 'N'}">미사용</c:if></td>
										</tr>
									</c:forEach>
								</c:otherwise>
							</c:choose>
					...
					<div id="search">
						<fieldset>
							<form:select path="keywordTp" style="width:100px;">
								<form:option value="1" label="코드" />
								<form:option value="2" label="코드명" />
							</form:select>
							<form:input type="text" size="30" path="sComCode" />
							<form:input type="text" size="30" path="codeName"
								style="display:none;" />
										...
										<td valign="top" bgcolor="#FFFFFF">
											...
											<div id="contents" style="width: 850px">
												<table border="0" cellspacing="0" cellpadding="0">
													<tr>
														<td><img src="images/title/title55.gif"></td>
													</tr>
													<tr>
														<td height="10"></td>
														<td></td>
												...
												<table width="100%" border="0" cellspacing="0"
													...
													<tr>
														<td><form:select path="keywordTp"
																...
																<form:option value="1" label="코드" />
																<form:option value="2" label="코드명" />
															</form:select> <form:input type="text" size="30" path="sComCode" /> <form:input
																type="text" size="30" path="codeName"
												...
												<table width="100%" border="0" cellspacing="0"
													...
													<tr>
														...
														<td width="20%" align="center">코드</td>

														<td width="20%" align="center">코드명</td>

														<td width="35%" align="center">코드설명</td>
															...
															<c:forEach var="list" items="${comCodeList}">
																<tr
																	...
																	<td width="20%" align="center"><input type="text"
																		...
																		value="${list.comCode}" readonly="readonly" /></td>

																	<td width="20%" align="center"><input type="text"
																		class="invisiblebox" style="width: 170px"
																		value="${list.codeName}" readonly="readonly" /></td>

																	<td width="35%" align="center"><input type="text"
																		class="invisiblebox" style="width: 290px"
																		value="${list.codeRemark}" readonly="readonly" /></td>

																	<td width="15%" align="center" style="padding: 7 0 7 0">
																		<c:if test="${list.useState == 'Y'}">
