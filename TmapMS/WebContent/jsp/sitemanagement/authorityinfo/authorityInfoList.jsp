...
<script type="text/javascript">
	...
	$(document).ready(function() {
	   ...
	   //조회 버튼 엔터 이벤트 처리
	    $("#authority").bind("keydown", function(event) {
	        if (event.keyCode == 13) {	
	        	searchPage(0);
	        }
	    });
	   
	    $("#menuAuthorityId").bind("keydown", function(event) {
	        if (event.keyCode == 13) {	
	        	searchPage(0);
	        }
	    });
	...
	function executeNew() {
		$("#ContentsForm").attr('action','authorityInfoNewForm.do');
	    $("#ContentsForm").submit();
	}
	
	function executeDetail(menuAuthorityId) {
		$("#menuAuthorityId").val(menuAuthorityId);
	...
	function searchCondChange(keywordTp) {
		if ($("#keywordTp").val() == "1") {
			$("#authority").val("");
			$("#authority").hide();
			$("#menuAuthorityId").show();
			
		} else {
	        $("#menuAuthorityId").val("");
	        $("#menuAuthorityId").hide();
	        $("#authority").show();
	...
	function searchPage(page) {
		...
		$("#ContentsForm").attr('action','authorityInfoList.do');
	    $("#ContentsForm").submit();
	}
...
<body>
	<form:form commandName="ContentsForm">
		...
		<form:hidden path="menuAuthorityId"/>
		
		<jsp:include page="../../common/menu.jsp" />
		<div id="bodyDiv">
			<div class="contentsDiv">
				<div class="contentDiv">
				<img src="images/h3_ico.gif"><span>권한정보 관리</span><br></br>
					<table class="sub_0601_table">
						...
						<tbody>
							...
							<c:choose>
								<c:when test="${empty authorityList}">
									<tr>
								...
								<c:otherwise>
									<c:forEach var="list" items="${authorityList}">
										<tr onmouseover="rowOnMouseOver(this);this.style.cursor='hand'" onMouseOut="rowOnMouseOut(this);" onclick="javascript:executeDetail('${list.menuAuthorityId}');">
											<td class="td03">${list.rownum}</td>
											<td class="td03">${list.menuAuthorityId}</td>
											<td class="td03">${list.authority}</td>
											<td class="td03">${list.etc}</td>
											<td class="td03">
												<c:if test="${list.useState == 'Y'}">사용	</c:if> 
												<c:if test="${list.useState == 'N'}">미사용</c:if>
											</td>
										</tr>
									</c:forEach>
								</c:otherwise>
					...
					<%-- <div id="search">
						<fieldset>
							<form:select path="keywordTp" style="width:100px;">
								<form:option value="1" label="권한ID" />
								<form:option value="2" label="권한명" />
							</form:select>
							<form:input type="text" size="30" path="menuAuthorityId" />
							<form:input type="text" size="30" path="authority"
								style="display:none;" />
										...
										<td valign="top" bgcolor="#FFFFFF">
											...
											<div id="contents" style="width: 850px">
												<table border="0" cellspacing="0" cellpadding="0">
													<tr>
														<td><img src="images/title/title61.gif"></td>
														<td width="30"></td>
													</tr>
													<tr>
														<td height="10"></td>
												...
												<div id="contents2">
													<table width="100%" border="0" cellspacing="0"
														...
														<tr>
															<td><form:select path="keywordTp"
																	...
																	path="menuAuthorityId" /> <form:input type="text"
																	size="30" path="authority" style="display:none;" /> <img
													...
													<table width="100%" border="0" cellspacing="0"
														...
														<tr>
															...
															<td width="20%" align="center">권한 아이디</td>
															<td width="20%" align="center">권한명</td>
															<td width="43%" align="center">권한설명</td>
															...
															<table width="100%" border="0" cellspacing="0"
																...
																<c:forEach var="list" items="${authorityList}">
																	<tr
																		...
																		onclick="javascript:executeDetail('${list.menuAuthorityId}');">
																		<td width="7%" align="center">${list.rownum}</td>
																		<td width="20%" align="center">${list.menuAuthorityId}</td>
																		<td width="20%" align="center">${list.authority}</td>
																		<td width="2%" align="center"></td>
																		<td width="41%" align="center">${list.etc}</td>
